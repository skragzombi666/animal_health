...
def test_new_context_specific_ai_modes_and_compact_ui_stay_available() -> None:
    backend = (INTEGRATION / "v086_features.py").read_text(encoding="utf-8")
    frontend = "".join(
        path.read_text(encoding="utf-8")
        for path in sorted(FRONTEND.glob("animal-health-panel.part*.js"))
    )

    assert '("general", "weight", "medication", "symptom")' in backend
