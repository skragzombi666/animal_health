...
(async()=>{
 ...
 const task={id:"TK-1",title:"Doxycyclin",task_kind:"medication",animal_id:"AH-1",animal_name:"Tina",recurrence_type:"daily",confirmation_mode:"required",planned:{medication_name:"Doxycyclin"}};
 panel.d.tasks=[task];
 const state=panel.calendarState097(task,"2026-08-18","2026-08-19");
 if(state.key!=="overdue"||state.occurrence!==null||state.date!=="2026-08-18")throw new Error("Virtual required-overdue calendar state is invalid");
 const icon=panel.calendarIcon097(task,state);
 if(!icon.includes('data-action="calendar-execute-099"')||!icon.includes('data-task-id="TK-1"')||!icon.includes('data-date="2026-08-18"'))throw new Error("Virtual overdue calendar item is not executable");
 panel.svc=async(service,payload,response)=>{
  if(service!=="list_task_occurrences"||payload.task_id!=="TK-1"||payload.from_date!=="2026-08-18"||payload.to_date!=="2026-08-18"||payload.status!=="all"||response!==true)throw new Error("Occurrence lookup payload is invalid");
 ...
 console.log("Animal Health calendar runtime validation passed");
