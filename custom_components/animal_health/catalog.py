...
def _resolve(filename: str, value: str | None) -> CatalogMatch | None:
    ...
    for item in document["items"]:
        if requested not in {_normalise(name) for name in _item_names(item)}:
            continue
        name = (
            item.get("name")
            or item.get("name_de")
...
def resolve_breed(value: str | None) -> CatalogMatch | None:
    return _resolve("breeds.json", value)
...
def canonical_breed_name(
    ...
    if value is None or not value.strip():
        return None, None
    match = resolve_breed(value)
    if match is None:
        return value.strip(), None
    expected_species = match.item.get("species_id")
    if species_id is not None and expected_species != species_id:
        raise ValueError(
            f"Breed {match.name} belongs to {expected_species}, not {species_id}"
        )
