...
async def async_migrate_v0927_task_kinds(hass: HomeAssistant) -> None:
...
def _migrate_task_record_configs_sync(database_path: Path) -> None:
    ...
    try:
        ...
        if row is None or all(kind in sql for kind in ("'deworming'", "'supplement'", "'feed'")):
            return
        columns = {
            str(item[1])
            for item in connection.execute("PRAGMA table_info(task_record_configs)").fetchall()
        }
        connection.executescript(
            ...
            CREATE TABLE task_record_configs_v0927 (
        ...
        if "confirmation_mode" in columns:
            connection.execute(
                """
                INSERT INTO task_record_configs_v0927 (
                    task_id, task_kind, template_json, confirmation_mode, created_at, updated_at
                )
                SELECT task_id, task_kind, template_json,
                       COALESCE(confirmation_mode, 'required'), created_at, updated_at
            ...
            connection.execute(
                ...
                SELECT task_id, task_kind, template_json, 'required', created_at, updated_at
                FROM task_record_configs
                """
            )
        connection.executescript(
            ...
            ALTER TABLE task_record_configs_v0927 RENAME TO task_record_configs;
        ...
        if violations:
            raise RuntimeError(
                "Animal Health 0.9.27 task-kind migration created foreign-key violations: "
                f"{violations}"
            )
        connection.commit()
    except Exception:
